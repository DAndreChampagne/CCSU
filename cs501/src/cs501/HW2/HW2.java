...
public class HW2 {
    ...
    public static class QuickSort implements ISortingAlgorithm {
		...
		private SortingAlgorithmResults Sort(int[] data, int low, int high) {
			int pivotPoint = Partition(data, low, high);
			
			if (low < pivotPoint) {
				Sort(data, low, pivotPoint-1);
			}
			
			if (high > pivotPoint) {
				Sort(data, pivotPoint+1, high);
			}
